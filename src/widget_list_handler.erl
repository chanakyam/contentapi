...
index(Req, State) ->
	...
	Url = case proplists:is_defined(<<"format">>, QueryTuples) of
		true ->
			contentapi_dbutils:get_widget( Limit, Skip, binary_to_list(proplists:get_value(<<"format">>, QueryTuples)));

		false ->
			contentapi_dbutils:get_widget( Limit, Skip, binary_to_list(?DEFAULT_VIDEOS_FORMAT))	
	end, 

	io:format("Url is from widget_list_handler ~p ~n " , [Url]),
	Url1 = binary_to_list(Url),
	io:format("url1 is ~p ~n", [Url1]),
	{ok, "200", _, Response1} = ibrowse:send_req(Url1,[],get,[],[]),
	io:format("Response is ~p ~n", [Response1]),

	Props = jsx:decode(list_to_binary(Response1)),
	...
	ResponseBody=jsx:encode([{<<"articles">>, Articles}]),
