...
index(Req, State) ->
	...
	ChannelDBView = "text_" ++ binary_to_list(Channel),
	...
	Url = case proplists:is_defined(<<"format">>, QueryTuples) of
		true ->
			contentapi_dbutils:get_news(ChannelDBView, Limit, Skip, binary_to_list(proplists:get_value(<<"format">>, QueryTuples)));

		false ->
			contentapi_dbutils:get_news(ChannelDBView, Limit, Skip, binary_to_list(?DEFAULT_NEWS_FORMAT))	
	end, 
